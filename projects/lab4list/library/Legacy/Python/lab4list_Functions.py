...
def REG_THRESHOLD_GET(handle):
    [err, data] = __abstracted_reg_read(lab4list_RegisterFile.SCI_REG_THRESHOLD, handle)
    return err, data

def REG_THRESHOLD_SET(data, handle):
    err = __abstracted_reg_write(data, lab4list_RegisterFile.SCI_REG_THRESHOLD, handle)
    return err

def REG_DELTA_GET(handle):
    [err, data] = __abstracted_reg_read(lab4list_RegisterFile.SCI_REG_DELTA, handle)
    return err, data

def REG_DELTA_SET(data, handle):
    err = __abstracted_reg_write(data, lab4list_RegisterFile.SCI_REG_DELTA, handle)
    return err

def REG_LOST_GET(handle):
    [err, data] = __abstracted_reg_read(lab4list_RegisterFile.SCI_REG_LOST, handle)
    return err, data

def REG_LOST_SET(data, handle):
    err = __abstracted_reg_write(data, lab4list_RegisterFile.SCI_REG_LOST, handle)
    return err

def REG_GOOD_GET(handle):
    [err, data] = __abstracted_reg_read(lab4list_RegisterFile.SCI_REG_GOOD, handle)
    return err, data

def REG_GOOD_SET(data, handle):
    err = __abstracted_reg_write(data, lab4list_RegisterFile.SCI_REG_GOOD, handle)
    return err
def REG_ANALOG_OFFSET_SET(data, handle):
    err = __abstracted_reg_write(data, lab4list_RegisterFile.SCI_REG_ANALOG_OFFSET, handle)
    return err



def OSCILLOSCOPE_Oscilloscope_0_START(handle):
    err = __abstracted_reg_write(0, lab4list_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, lab4list_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    return True

def OSCILLOSCOPE_Oscilloscope_0_SET_DECIMATOR(OscilloscopeDecimator, handle):
    err = __abstracted_reg_write(OscilloscopeDecimator, lab4list_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_DECIMATOR, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_PRETRIGGER(OscilloscopePreTrigger, handle):
    err = __abstracted_reg_write(OscilloscopePreTrigger, lab4list_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_PRETRIGGER, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_LEVEL(OscilloscopeTriggerLevel, handle):
    err = __abstracted_reg_write(OscilloscopeTriggerLevel, lab4list_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_LEVEL, handle)
    return err
...
def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_MODE(OscilloscopeTriggerMode, OscilloscopeTriggerChannel, OscilloscopeTriggerEdge, handle):
    ...
    err = __abstracted_reg_write(triggermode, lab4list_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_MODE, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(lab4list_RegisterFile.SCI_REG_Oscilloscope_0_READ_STATUS, handle)
    return err, status

def OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle):
    [err, position] = __abstracted_reg_read(lab4list_RegisterFile.SCI_REG_Oscilloscope_0_READ_POSITION, handle)
    return err, position

def OSCILLOSCOPE_Oscilloscope_0_GET_DATA(timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(1024, lab4list_RegisterFile.SCI_REG_Oscilloscope_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def LISTMODULE_List_0_RESET(handle):
    err = __abstracted_reg_write(2, lab4list_RegisterFile.SCI_REG_List_0_CONFIG, handle)
    return err
...
def LISTMODULE_List_0_START(handle):
    ...
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, lab4list_RegisterFile.SCI_REG_List_0_CONFIG, handle)
    if (err != 0):
       return False
    return True

def LISTMODULE_List_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(lab4list_RegisterFile.SCI_REG_List_0_STATUS, handle)
    flags = status & 0xf
    data_available = status >> 8
    return err, flags, data_available

def LISTMODULE_List_0_GET_DATA(Data_Number, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_fifo_read(Data_Number, lab4list_RegisterFile.SCI_REG_List_0_FIFOADDRESS, lab4list_RegisterFile.SCI_REG_List_0_STATUS,1, timeout_ms, handle)
    return err, data, read_data, valid_data
