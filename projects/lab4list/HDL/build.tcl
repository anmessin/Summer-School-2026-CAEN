...
puts $outfile1 "       <filename>top_lab4list.bin</filename>"
puts $outfile1 "   </FPGA>"
...
create_project lab4list $outputDir -part XC7S25CSGA225-1 -force
...
add_files -force ./top_lab4list.vhd
...
set_property Top top_lab4list [current_fileset]
...
file copy -force $outputDir/lab4list.runs/impl_1/top_lab4list.bin $artDir/top_lab4list.bin
file copy -force $script_path/RegisterFile.json $artDir/RegisterFile.json

if { [catch { exec rm $artDir/lab4list.niu  } msg] } {
puts "No niu file present to be deleted"
}

exec zip -j $artDir/lab4list.niu $artDir/RegisterFile.json $artDir/top_lab4list.bin $artDir/content.xml
