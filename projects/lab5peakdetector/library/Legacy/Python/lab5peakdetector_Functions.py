...
import lab5peakdetector_RegisterFile
from ctypes import *
import array
import numpy as np
import os
...
def REG_EL_M_GET(handle):
    [err, data] = __abstracted_reg_read(lab5peakdetector_RegisterFile.SCI_REG_EL_M, handle)
    return err, data

def REG_EL_M_SET(data, handle):
    err = __abstracted_reg_write(data, lab5peakdetector_RegisterFile.SCI_REG_EL_M, handle)
    return err

def REG_PEAK_DELAY_GET(handle):
    [err, data] = __abstracted_reg_read(lab5peakdetector_RegisterFile.SCI_REG_PEAK_DELAY, handle)
    return err, data

def REG_PEAK_DELAY_SET(data, handle):
    err = __abstracted_reg_write(data, lab5peakdetector_RegisterFile.SCI_REG_PEAK_DELAY, handle)
    return err

def REG_BL_M_GET(handle):
    [err, data] = __abstracted_reg_read(lab5peakdetector_RegisterFile.SCI_REG_BL_M, handle)
    return err, data

def REG_BL_M_SET(data, handle):
    err = __abstracted_reg_write(data, lab5peakdetector_RegisterFile.SCI_REG_BL_M, handle)
    return err

def REG_BL_HOLD_GET(handle):
    [err, data] = __abstracted_reg_read(lab5peakdetector_RegisterFile.SCI_REG_BL_HOLD, handle)
    return err, data

def REG_BL_HOLD_SET(data, handle):
    err = __abstracted_reg_write(data, lab5peakdetector_RegisterFile.SCI_REG_BL_HOLD, handle)
    return err
def REG_ANALOG_OFFSET_SET(data, handle):
    err = __abstracted_reg_write(data, lab5peakdetector_RegisterFile.SCI_REG_ANALOG_OFFSET, handle)
    return err



def LISTMODULE_List_0_RESET(handle):
    err = __abstracted_reg_write(2, lab5peakdetector_RegisterFile.SCI_REG_List_0_CONFIG, handle)
    return err
...
def LISTMODULE_List_0_START(handle):
    ...
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, lab5peakdetector_RegisterFile.SCI_REG_List_0_CONFIG, handle)
    if (err != 0):
       return False
    return True

def LISTMODULE_List_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(lab5peakdetector_RegisterFile.SCI_REG_List_0_STATUS, handle)
    flags = status & 0xf
    data_available = status >> 8
    return err, flags, data_available

def LISTMODULE_List_0_GET_DATA(Data_Number, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_fifo_read(Data_Number, lab5peakdetector_RegisterFile.SCI_REG_List_0_FIFOADDRESS, lab5peakdetector_RegisterFile.SCI_REG_List_0_STATUS,1, timeout_ms, handle)
    return err, data, read_data, valid_data
