...
SCILIB int REG_INT_Q_GET(uint32_t *val, NI_HANDLE *handle)
{
     return __abstracted_reg_read(val, SCI_REG_INT_Q, handle);
}
SCILIB int REG_INT_Q_SET(uint32_t val, NI_HANDLE *handle)
{
     return __abstracted_reg_write(val, SCI_REG_INT_Q, handle);
}
