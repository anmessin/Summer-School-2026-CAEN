...
def REG_THRESHOLD_GET(handle):
    [err, data] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_THRESHOLD, handle)
    return err, data

def REG_THRESHOLD_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_THRESHOLD, handle)
    return err

def REG_M_LENGTH_GET(handle):
    [err, data] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_M_LENGTH, handle)
    return err, data

def REG_M_LENGTH_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_M_LENGTH, handle)
    return err

def REG_BL_HOLD_GET(handle):
    [err, data] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_BL_HOLD, handle)
    return err, data

def REG_BL_HOLD_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_BL_HOLD, handle)
    return err

def REG_INT_Q_GET(handle):
    [err, data] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_INT_Q, handle)
    return err, data

def REG_INT_Q_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_INT_Q, handle)
    return err

def REG_GAIN_GET(handle):
    [err, data] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_GAIN, handle)
    return err, data

def REG_GAIN_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_GAIN, handle)
    return err

def REG_INT_SAMPLES_GET(handle):
    [err, data] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_INT_SAMPLES, handle)
    return err, data

def REG_INT_SAMPLES_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_INT_SAMPLES, handle)
    return err

def REG_PRE_TRIGGER_GET(handle):
    [err, data] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_PRE_TRIGGER, handle)
    return err, data

def REG_PRE_TRIGGER_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_PRE_TRIGGER, handle)
    return err

def REG_DELAY_GET(handle):
    [err, data] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_DELAY, handle)
    return err, data

def REG_DELAY_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_DELAY, handle)
    return err
def REG_ANALOG_OFFSET_SET(data, handle):
    err = __abstracted_reg_write(data, lab9exam_RegisterFile.SCI_REG_ANALOG_OFFSET, handle)
    return err



def SPECTRUM_Spectrum_0_RESET(handle):
    err = __abstracted_reg_write(2, lab9exam_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_START(handle):
    err = __abstracted_reg_write(4, lab9exam_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_FLUSH(handle):
    err = __abstracted_reg_write(1, lab9exam_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err

def SPECTRUM_Spectrum_0_STOP(handle):
    err = __abstracted_reg_write(0, lab9exam_RegisterFile.SCI_REG_Spectrum_0_CONFIG, handle)
    return err
...
def SPECTRUM_Spectrum_0_SET_PARAMETERS(Bin, AcquisitionMode, TargetValue, handle):
    ...
    if (Bin == 128):
        rebin_value= 7
    err = __abstracted_reg_write(rebin_value, lab9exam_RegisterFile.SCI_REG_Spectrum_0_CONFIG_REBIN, handle)
    acq_mode=0
    if (AcquisitionMode =="Event"):
        acq_mode=(1 << 30) + TargetValue
    if (AcquisitionMode =="Time"):
        acq_mode=(1 << 31) + TargetValue*1000
    err = __abstracted_reg_write(acq_mode, lab9exam_RegisterFile.SCI_REG_Spectrum_0_CONFIG_LIMIT, handle)
    return err

def SPECTRUM_Spectrum_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_Spectrum_0_STATUS, handle)
    return err, status

def SPECTRUM_Spectrum_0_GET_DATA(Bin, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(Bin, lab9exam_RegisterFile.SCI_REG_Spectrum_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data



def OSCILLOSCOPE_Oscilloscope_0_START(handle):
    err = __abstracted_reg_write(0, lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    return True

def OSCILLOSCOPE_Oscilloscope_0_SET_DECIMATOR(OscilloscopeDecimator, handle):
    err = __abstracted_reg_write(OscilloscopeDecimator, lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_DECIMATOR, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_PRETRIGGER(OscilloscopePreTrigger, handle):
    err = __abstracted_reg_write(OscilloscopePreTrigger, lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_PRETRIGGER, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_LEVEL(OscilloscopeTriggerLevel, handle):
    err = __abstracted_reg_write(OscilloscopeTriggerLevel, lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_LEVEL, handle)
    return err
...
def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_MODE(OscilloscopeTriggerMode, OscilloscopeTriggerChannel, OscilloscopeTriggerEdge, handle):
    ...
    err = __abstracted_reg_write(triggermode, lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_MODE, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_READ_STATUS, handle)
    return err, status

def OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle):
    [err, position] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_READ_POSITION, handle)
    return err, position

def OSCILLOSCOPE_Oscilloscope_0_GET_DATA(timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(3072, lab9exam_RegisterFile.SCI_REG_Oscilloscope_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
...
def LISTMODULE_List_0_RESET(handle):
    err = __abstracted_reg_write(2, lab9exam_RegisterFile.SCI_REG_List_0_CONFIG, handle)
    return err
...
def LISTMODULE_List_0_START(handle):
    ...
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, lab9exam_RegisterFile.SCI_REG_List_0_CONFIG, handle)
    if (err != 0):
       return False
    return True

def LISTMODULE_List_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(lab9exam_RegisterFile.SCI_REG_List_0_STATUS, handle)
    flags = status & 0xf
    data_available = status >> 8
    return err, flags, data_available

def LISTMODULE_List_0_GET_DATA(Data_Number, timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_fifo_read(Data_Number, lab9exam_RegisterFile.SCI_REG_List_0_FIFOADDRESS, lab9exam_RegisterFile.SCI_REG_List_0_STATUS,1, timeout_ms, handle)
    return err, data, read_data, valid_data
