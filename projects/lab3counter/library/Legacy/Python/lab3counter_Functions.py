...
import lab3counter_RegisterFile
from ctypes import *
import array
import numpy as np
import os
...
def REG_THRESHOLD_GET(handle):
    [err, data] = __abstracted_reg_read(lab3counter_RegisterFile.SCI_REG_THRESHOLD, handle)
    return err, data

def REG_THRESHOLD_SET(data, handle):
    err = __abstracted_reg_write(data, lab3counter_RegisterFile.SCI_REG_THRESHOLD, handle)
    return err

def REG_INIBIT_GET(handle):
    [err, data] = __abstracted_reg_read(lab3counter_RegisterFile.SCI_REG_INIBIT, handle)
    return err, data

def REG_INIBIT_SET(data, handle):
    err = __abstracted_reg_write(data, lab3counter_RegisterFile.SCI_REG_INIBIT, handle)
    return err

def REG_COUNTS_GET(handle):
    [err, data] = __abstracted_reg_read(lab3counter_RegisterFile.SCI_REG_COUNTS, handle)
    return err, data

def REG_COUNTS_SET(data, handle):
    err = __abstracted_reg_write(data, lab3counter_RegisterFile.SCI_REG_COUNTS, handle)
    return err

def REG_DELTA_GET(handle):
    [err, data] = __abstracted_reg_read(lab3counter_RegisterFile.SCI_REG_DELTA, handle)
    return err, data

def REG_DELTA_SET(data, handle):
    err = __abstracted_reg_write(data, lab3counter_RegisterFile.SCI_REG_DELTA, handle)
    return err

def REG_RESET_GET(handle):
    [err, data] = __abstracted_reg_read(lab3counter_RegisterFile.SCI_REG_RESET, handle)
    return err, data
...
def OSCILLOSCOPE_Oscilloscope_0_START(handle):
    err = __abstracted_reg_write(0, lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    err = __abstracted_reg_write(1, lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_ARM, handle)
    if (err != 0):
       return False
    return True

def OSCILLOSCOPE_Oscilloscope_0_SET_DECIMATOR(OscilloscopeDecimator, handle):
    err = __abstracted_reg_write(OscilloscopeDecimator, lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_DECIMATOR, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_PRETRIGGER(OscilloscopePreTrigger, handle):
    err = __abstracted_reg_write(OscilloscopePreTrigger, lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_PRETRIGGER, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_LEVEL(OscilloscopeTriggerLevel, handle):
    err = __abstracted_reg_write(OscilloscopeTriggerLevel, lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_LEVEL, handle)
    return err
...
def OSCILLOSCOPE_Oscilloscope_0_SET_TRIGGER_MODE(OscilloscopeTriggerMode, OscilloscopeTriggerChannel, OscilloscopeTriggerEdge, handle):
    ...
    err = __abstracted_reg_write(triggermode, lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_CONFIG_TRIGGER_MODE, handle)
    return err

def OSCILLOSCOPE_Oscilloscope_0_GET_STATUS(handle):
    [err, status] = __abstracted_reg_read(lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_READ_STATUS, handle)
    return err, status

def OSCILLOSCOPE_Oscilloscope_0_GET_POSITION(handle):
    [err, position] = __abstracted_reg_read(lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_READ_POSITION, handle)
    return err, position

def OSCILLOSCOPE_Oscilloscope_0_GET_DATA(timeout_ms, handle):
    [err, data, read_data, valid_data] = __abstracted_mem_read(1024, lab3counter_RegisterFile.SCI_REG_Oscilloscope_0_FIFOADDRESS, timeout_ms, handle)
    return err, data, read_data, valid_data
